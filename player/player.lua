...
local takeDmgCollisionFilter = function(item, other)
  if item.owner == other.owner then return nil
  elseif (other.layer == LAYER.ATKBOX) and (other.active) then return "cross"
  else return nil
  end
end
...
local dealDmgCollisionFilter = function(item, other)
  if item.owner == other.owner then return nil
  elseif (other.layer == LAYER.HURTBOX) and (other.active) then return "cross"
  else return nil
  end
end
...
function Player:update(dt)
  ...
  --       if not tableContains(self.guardBox.targetTable, other) then
  --         table.insert(self.guardBox.targetTable, other)
  --       end
  --       COUNTER = COUNTER + 1
